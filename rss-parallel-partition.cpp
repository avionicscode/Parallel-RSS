...
void readTextPointList(int n, int d, std::string strFileName, std::vector<double *> &result)
{
...
void skyline_update(std::vector<double *> &data, size_t width, size_t window, size_t start, size_t end)
{
...
auto main(int argc, char **argv) -> int
{
  ...
  readTextPointList(window, dimensionality, filename, data);

  size_t core_number = 4;
  size_t step = window / core_number + 1;

#pragma omp parallel for schedule(dynamic)
  for (int i = core_number - 1; i >= 0; i--)
  {
    skyline_update(data, dimensionality, window, i * step, std::min((unsigned short)((i + 1) * step), (unsigned short)window));
  }
...
// g++ -O3 -m64 -std=c++17 -fopenmp rss-parallel-partition.cpp

/*
if window = 100 and core_number = 4;
step = window / core_number + 1;// step = 100/ 4 + 1 = 26;
 ...
 skyline_update(data, dimensionality, window, i * step, std::min((unsigned short)((i + 1) * step), (unsigned short)window));
  ...
  skyline_update(data,8,100, 0 , 26)
*/