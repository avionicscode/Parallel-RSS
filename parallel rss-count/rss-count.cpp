...
auto par_run_skyline(const char *name, size_t dimensionality, size_t window, const char *stream) -> bool
{
  ...
  if (stream)
  {
    ...
    double stopTime1 = omp_get_wtime();
    double secsElapsed1 = stopTime1 - startTime1;
    std::cout << "Parallel Skyline: " << secsElapsed1 << std::endl;
...
auto main(int argc, char **argv) -> int
{
  ...
  par_run_skyline("RSS-COUNT", dimensionality, window, stream);
  return 0;
}